...
const BlogLayout: React.FunctionComponent<Props> = ({ title, date, titleComponent = 'h2', category, tag }) => {
      ...
      <Container maxWidth="md" className={classes.blog}>
        <div className={classes.blogTitle}>
          ...
          {tag &&
            tag.length &&
            tag.map((t) => (
              <Chip key={t} variant="outlined" size="small" label={`#${t}`} color="primary" className="chip" />
            ))}
        </div>
      </Container>
    </section>
